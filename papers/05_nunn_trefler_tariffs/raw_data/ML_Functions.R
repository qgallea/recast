...
lassoF <- function(datause, dataout, form_x, form_y, logit=FALSE, alp=alp, arg=arg, s=s){
  ...
  yhatuse         <- predict(lasso, newx=fit.p$x[,-1]) # eigen aanpassing
  ...
  yhatout         <- predict(lasso, newx=fit.p$x[,-1]) # eigen aanpassing
...
nnetF <- function(datause, dataout, form_x, form_y, clas=FALSE, arg){
  ...
  for (s in 1:length(scales)) {
    if (length(unique(datause[,s]))<3) {
      scales[s] = 1
    }
  }
  print(scales)
  
  datause[,!f] <- as.data.frame(scale(datause[,!f], center = mins, scale = scales))
  dataout[,!f] <- as.data.frame(scale(dataout[,!f], center = mins, scale = scales))
  
  # linout = TRUE # dit heb ik toegevoegd
  
  # print(linout)
  
  # datause <- datause[,which(apply(datause, 2, var) != 0)]
  # print(head(datause))
  

  form           <- as.formula(paste(form_y, "~", form_x))
  nn             <- do.call(nnet, append(list(formula=form, data=datause, linout=linout), arg))
  k              <- which(colnames(dataout)==form_y) # selecteer index van dependent var
...
boost <- function(datause, dataout, form_x, form_y, bag.fraction = .5, interaction.depth=2, n.trees=1000, shrinkage=.01, distribution='gaussian', option){
  ...
  # # cross-validatie voor parameters
  # library(caret)
  # train.control <- trainControl(method = "cv", number = 10)
  # print("Heere")
  # model <- train(form, data = datause, method = "gbm", trControl = train.control)
  # option$n.trees <- as.numeric(model$bestTune[1])
  # option$interaction.depth <- as.numeric(model$bestTune[2])
  # option$shrinkage <- as.numeric(model$bestTune[3])
  # option$n.minobsinnode <- as.numeric(model$bestTune[4])
  # print(option)
  option$n.minobsinnode <- 3
  print(option)
  ...
  # print(boostfit)
  # 
  # print(as.numeric(model$bestTune[[1]]))
  # print(as.numeric(model$bestTune[2]))
  # print(as.numeric(model$bestTune[3]))
  # print(as.numeric(model$bestTune[4]))
