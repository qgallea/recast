...
#  Modified: returning Lasso coefficients                                                               #
...
DoubleML <- function(data, y, d,z, xx, xL, methods, DML, nfold, est, arguments, ensemble, silent=FALSE, trim){
  ...
  lasso_y_coefficients <- list()
  lasso_d_coefficients <- list()
  lasso_z_coefficients <- list() 
  ...
  if(TRUE %in% (methods %in% c("Lasso"))){
    ...
    for(i in 1:nfold){
      lasso_y_coefficients[[i]] <-coef(cond.comp[[lasso_index,i]]$fit.y$model$glmnet.fit, s= cond.comp[[lasso_index,i]]$fit.y$model$lambda.min)
      lasso_d_coefficients[[i]] <-  coef(cond.comp[[lasso_index,i]]$fit.z$model$glmnet.fit, s= cond.comp[[lasso_index,i]]$fit.z$model$lambda.min)
  
      if(!is.null(z)){
        lasso_z_coefficients[[i]] <-  coef(cond.comp[[lasso_index,i]]$fit.z2$model$glmnet.fit, s= cond.comp[[lasso_index,i]]$fit.z2$model$lambda.min)
      }
    }
  }
  #y_hat <- matrix(list(),length(methods),nfold)
  #y_hat[[1,1]] <- methods
  #d_hat <- matrix(list(),length(methods),nfold)
  #d_hat[[1,1]] <- methods
  #z_hat <- matrix(list(),length(methods),nfold)
  #z_hat[[1,1]] <- methods
  #y_res <- matrix(list(),length(methods)+1,nfold)
  #y_res[[1,1]] <- methods
  #d_res <- matrix(list(),length(methods)+1,nfold)
  #d_res[[1,1]] <- methods
  #z_res <- matrix(list(),length(methods)+1,nfold)
  #z_res[[1,1]] <- methods
  
 # if(length(methods)>1){
    #for(k in 1:(length(methods)-1)){
     # for(j in 1:nfold){
      #  y_hat[[k+1,j]] <- cond.comp[[k,j]]$my_x
       # d_hat[[k+1,j]] <- cond.comp[[k,j]]$mz_x 
       # if(!is.null(z)){z_hat[[k+1,j]] <- cond.comp[[k,j]]$mz2_x}
       # else{z_hat <- NULL}
      #}
    #}
    #for(k in 1:length(methods)){
      #for(j in 1:nfold){
       # y_res[[k+1,j]] <- cond.comp[[k,j]]$ry
       # d_res[[k+1,j]] <- cond.comp[[k,j]]$rz 
       # if(!is.null(z)){z_res[[k+1,j]] <- cond.comp[[k,j]]$rz2}
       # else{z_res <- NULL}
     # }
   # }
...
cond_comp <- function(datause, dataout, y, d, z=NULL, x, method, plinear,xL, binary,flag=0, arguments){
  ...
  return(list(method = method, fit.dz0 = fit.dz0, err.dz0=err.dz0, md_z0x=md_z0x, fit.dz1 = fit.dz1, err.dz1=err.dz1, md_z1x=md_z1x, rz2=rz2, mz2_x=mz2_x, err.z2=err.z2, my_z1x=my_z1x, mz_x= mz_x, my_z0x=my_z0x, my_x = my_x, err.z = err.z,  err.yz0= err.yz0,  err.yz1=err.yz1, mis.z=mis.z, ry=ry , rz=rz, err.y=err.y,  fit.y=fit.y, fit.z= fit.z, fit.z2 = fit.z2, fit.yz1out= fit.yz1$yhatout,  fit.yz0out= fit.yz0$yhatout));
