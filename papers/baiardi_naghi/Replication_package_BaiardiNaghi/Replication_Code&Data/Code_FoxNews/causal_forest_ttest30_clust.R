...
X <- as.matrix(df[,c(census_controls_90, census_controls_20, cable_controls, controls_VI)]) 

# create list of variable names
X.names <- c(census_controls_90, census_controls_20, cable_controls,controls_VI)
...
Y.forest = regression_forest(X, Y, clusters = df$diststate, sample.weights = df$totpresvotes1996, num.trees=2000)
Y.hat = predict(Y.forest)$predictions
W.forest = regression_forest(X, W, clusters = df$diststate, sample.weights = df$totpresvotes1996, num.trees = 2000)
...
########################## Single Forest #############################
# train causal forest
cf = causal_forest(X, Y, W, Y.hat = Y.hat, W.hat = W.hat, sample.weights = df$totpresvotes1996, 
                   clusters = df$diststate, tune.parameters = "all", num.trees = 2000)
...
print(xtable(ate_matrix, type="latex"), include.rownames=FALSE, file=paste("FN_T3_C2.txt"))
...
print(xtable(as.matrix(cbind(X.names[selected], paste0(round(varimp[selected]*100,2), "%"))), type="latex"), include.rownames=FALSE, file=paste("FN_S10.txt"))

####################
# TABLE 4 Panel B and TABLE S3.8 #
####################
...
kable(data_matrix)
print(xtable(data_matrix, type="latex"), include.rownames=FALSE, file=paste("FN_T4_PB_&_S8.txt"))

##############
# TABLE S3.6 Column 2 #
##############
print("CALIBRATION TEST")
capture.output(test_calibration(cf), file=paste("FN_S6_C2.txt"))

########################## Double Forest #############################
# variable selection
#cf.raw = causal_forest(X, Y, W,
#                       Y.hat = Y.hat, W.hat = W.hat, sample.weights = df$totpresvotes1996,
#                       clusters = df$diststate, tune.parameters="all", num.trees = 2000)
varimp = variable_importance(cf)
...
cf = causal_forest(X[,selected.idx], Y, W, Y.hat = Y.hat, W.hat = W.hat, sample.weights = df$totpresvotes1996,  ## X[,selected.idx] for Double Forest, X for Single Forest 
                   clusters = df$diststate, tune.parameters = "all", num.trees = 2000)
tau.hat = predict(cf)$predictions
...
#plot(sort(variable_importance(cf.raw)[1:75], decreasing=TRUE), ylab="Importance", type = "l")
...
print(xtable(ate_matrix, type="latex"), include.rownames=FALSE, file=paste("FN_S11_C2.txt"))
...
kable(data_matrix)
print(xtable(data_matrix, type="latex"), include.rownames=FALSE, file=paste("FN_S12_PB.txt"))
